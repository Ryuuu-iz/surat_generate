<?php
/**
 * proses_surat_perintah.php
 * -----------------------------------------------------------------
 * Backend pemroses untuk jenis surat "Surat Perintah".
 * Alur:
...
define('TEMPLATE_FILE', TEMPLATE_DIR . 'surat_perintah.rtf');
...
$data = [
    ...
    'pertimbangan' => ambil('pertimbangan'), // HTML dari CKEditor
    'dasar'        => ambil('dasar'),        // HTML dari CKEditor
    'kepada'       => ambil('kepada'),       // HTML dari CKEditor
    'untuk'        => ambil('untuk'),        // HTML dari CKEditor
    'bulan_tahun'  => ambil('bulan_tahun'),
    'nama'         => ambil('nama'),
 ...
 * -----------------------------------------------------------------
 * $noSpacing: jika true, list items tanpa spasi (rapat); jika false, dengan spasi
 */
function htmlToRtf($html, $noSpacing = false) {
    if (trim($html) === '') {
        return '';
    }
    ...
    $html = preg_replace_callback('/<ol[^>]*>(.*?)<\/ol>/is', function ($m) {
        ...
        $itemsHtml = preg_replace_callback('/<li[^>]*>(.*?)<\/li>/is', function ($li) use (&$counter) {
            $counter++;
            return "OL_ITEM:" . $counter . "" . $li[1] . "LIEND";
        }, $itemsHtml);
        return $itemsHtml;
    }, $html);
...
$rtfValues = [
    ...
    'pertimbangan' => htmlToRtf($data['pertimbangan']),
    'dasar'        => htmlToRtf($data['dasar']),
    'kepada'       => htmlToRtf($data['kepada']),
    'untuk'        => htmlToRtf($data['untuk']),
    'bulan_tahun'  => escapeRtfPlainText($data['bulan_tahun']),
    'nama'         => escapeRtfPlainText($data['nama']),
    'jabatan'      => escapeRtfPlainText($data['jabatan']),
    'pangkat_nrp'  => escapeRtfPlainText($data['pangkat_nrp']),
    'tembusan'     => htmlToRtf($data['tembusan'], true),
];
...
if (!file_exists(TEMPLATE_FILE)) {
    ...
    die('Template surat_perintah.rtf tidak ditemukan di folder templates/.');
}

$template = file_get_contents(TEMPLATE_FILE);

// PENTING: Word menyimpan RTF dengan word-wrap pada baris-baris panjang.
// Placeholder seperti ${bulan_tahun} bisa "terpotong" oleh newline mentah
// (mis. "$\{\nbulan_tahun\}"). Newline mentah TIDAK signifikan dalam RTF
...
// =====================================================================
$namaFileDasar = 'Surat_Perintah_' . preg_replace('/[^A-Za-z0-9_-]/', '_', $data['nomor_surat'] ?: date('Ymd_His'));
...
// =====================================================================
// FUNGSI: konversi ke PDF asli via COM Word.Application, dengan fallback
// otomatis ke RTF apabila COM gagal (mis. dijalankan di server non-Windows
// atau MS Word tidak terpasang / tidak dapat diinisialisasi).
// =====================================================================
function kirimFilePdfViaWordCOM($rtfContent, $namaFileDasar) {
    ...
    // 6.2. Pastikan ekstensi COM tersedia (hanya ada di PHP untuk Windows)
    if (class_exists('COM')) {
        $word = null;
        $doc  = null;
        try {
    ...
    // =================================================================
    // 6.3. Jika konversi PDF berhasil -> kirim PDF ke browser
    // =================================================================
    ...
    // =================================================================
    // 6.4. FALLBACK: COM gagal / tidak tersedia -> otomatis kirim RTF
    // =================================================================
    @unlink($tempRtfPath);
    @unlink($tempPdfPath);
